...
void QuickSort(int* pNumber, int iStart, int iEnd)
{
	...
	while (iS <= iE)
	{
		if (pNumber[iS] <= pNumber[iPivot])
			iS++;
		else if (pNumber[iS] > pNumber[iPivot])
		{
			if (pNumber[iE] > pNumber[iPivot])
				iE--;
			else if (pNumber[iE] < pNumber[iPivot])
			{
				Swap(&pNumber[iE], &pNumber[iS]);
			}
		}
	}

	//왼쪽
	QuickSort(pNumber, iStart, iS - 1);

	//오른쪽
	QuickSort(pNumber, iS + 1, iEnd);
